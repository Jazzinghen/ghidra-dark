...
class TCDBrowser:
    ...
    def update(self, preferences):
        ...
        categories = [
            _ for _ in tool_options.iter() if _.tag == "CATEGORY" and "NAME" in _.attrib
        ]
            ...
            for preference, option in preferences.get(category.get("NAME"), {}).items():
                ...
                if len(element) == 0:
                    ...
                    if isinstance(option, Wrapped):
                        for state in option.states:
                            e = [
                                _ for _ in element.iter() if _.get("NAME") == state.name
                            ][0]
        ...
        xmlstr = MD.parseString(ET.tostring(self.root)).toprettyxml(
